  ...
  return (
    <LampContainer lightOn={on}>
      ...
      <button
        ...
        className="mt-8 px-6 py-2 rounded-full bg-cyan-600 text-white font-semibold shadow-lg hover:bg-cyan-700 transition"
      >
        Turn {on ? "Off" : "On"}
      </button>
    </LampContainer>
  ...
  return (
    ...
    >
      <div className="relative flex w-full flex-1 scale-y-100 items-center justify-center isolate z-0 ">
        {/* Lamp light gradients, opacity controlled by lightOn */}
        <motion.div
          initial={{ opacity: 0.5, width: "15rem" }}
          whileInView={{ opacity: 1, width: "30rem" }}
          animate={{ opacity: lightOn ? 1 : 0.1 }}
          transition={{
            delay: 0.3,
            duration: 0.8,
            ease: "easeInOut",
        ...
        <motion.div
          ...
          className="absolute inset-auto left-1/2 h-30 w-[30rem] bg-gradient-conic from-transparent via-transparent to-cyan-500 text-white [--conic-position:from_290deg_at_center_top]"
        >
        ...
        {/* Lamp glow, opacity controlled by lightOn */}
        <div
          className={cn(
            "absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-cyan-500 blur-3xl transition-opacity duration-500",
            lightOn ? "opacity-50" : "opacity-10"
          )}
        ></div>
        <motion.div
          initial={{ width: "8rem" }}
          ...
          className="absolute inset-auto z-30 h-18 w-64 -translate-y-[6rem] rounded-full bg-cyan-400 blur-2xl"
        ></motion.div>
        <motion.div
          initial={{ width: "15rem" }}
      ...
      <div className="relative z-50 flex -translate-y-1/2 flex-col items-center px-5">
        {children}
      </div>
    </div>
  );
